...
def ViT_DELSYStraining(opt, wandb_set, model_dir, emg, label, patch_size, MODEL_DIM,
    ...
    # dataset division
    angle_value = np.mean(np.array(dataset[:][1]), axis=1)
    angle_range = np.linspace(round(min(angle_value), 2),
                              round(max(angle_value), 2),
                              num=20, endpoint=False)
    angle_interval = round(angle_range[1] - angle_range[0], 3)
    RANDOM_STATE = 42
