...
for i in range(1, 66):
   for j in range(len(class_list[i])):
      class_list[0].append(class_list[i][j])
class_list[0].sort()
print(test_list[0])
print(class_list[0])
test_emg = emg[torch.tensor(test_list[0])]
test_labels = labels[torch.tensor(test_list[0])]
for i in range(len(test_list[0])):
    class_list[0].remove(test_list[0][i])
print(class_list[0])
train_labels = labels[torch.tensor(class_list[0])]
train_emg = emg[torch.tensor(class_list[0])]
train_emg_filePath = 'F:/1024/train_emg.p'
train_labels_filePath = 'F:/1024/train_labels.p'
test_emg_filePath = 'F:/1024/test_emg.p'
test_labels_filePath = 'F:/1024/test_labels.p'
with open(train_emg_filePath, 'wb') as f:
   pickle.dump(train_emg, f)
with open(test_emg_filePath, 'wb') as f:
   pickle.dump(test_emg, f)
with open(train_labels_filePath, 'wb') as f:
   pickle.dump(train_labels, f)
with open(test_labels_filePath, 'wb') as f:
...
def train(args, loader, wandb):
  ...
  for epoch in range(1, epochs + 1):
    ...
    print(f'{epoch} / {epochs}. Time : {end - start}. Total loss : {avg_loss / len(loader)}')

  print(f'Training completed! Total training time : {total_time}')
  torch.save(model.state_dict(), 'F:/UROP/model.pt')

def test(args, loader, wandb):
  model = EMGRU(args).to(args['device'])
  model.load_state_dict(torch.load('F:/UROP/model.pt'))
  ...
  for x, y_true in loader:
      ...
      out, h = model(x.to(args['device']).float(), h)
      test_loss += loss_func(out, y_true.to(args['device'])).item()
      pred = out.max(1, keepdim = True)[1]
      correct += pred.eq(y_true.to(args['device']).view_as(pred)).sum().item()
      total += 1
  end = time.time()
