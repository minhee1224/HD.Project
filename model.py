...
def train(model, train_loader, scheduler, epoch, criterion, device, opt):
    ...
    for batch_idx, (emg, label) in tqdm_bar:
        ...
        if (opt.log_interval > 0) and ((batch_idx + 1) % opt.log_interval == 0):
            wandb.log(
                {
                    "Training Loss": round(train_loss/opt.log_interval, 4)    
                }
            )
...
def evaluate(model, test_loader, criterion, device, opt):
    ...
    with torch.no_grad():
        for batch_idx, (emg, label) in tqdm_bar:
            ...
            tqdm_bar.set_description(
                "Validation step: {} || Validation loss: {:.6f} || Validation accuracy: {:.6f}".format((batch_idx + 1)/len(test_loader), loss.item(), (n_correct/total)))
            # wandb logging
            wandb.log(
                {
                    'Validation Loss':
...
def ViTtraining(opt, model_dir, emg, label, patch_size, MODEL_DIM, HIDDEN_DIM,
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        lr = scheduler.get_lr()
        print("\n[EPOCH: {:2d}], \tModel: ViT, \tLR: {:8.5f}, ".format(
            epoch, lr) \
